function createUser() {
	 ...
	 * @param {string | null} [id] - Optional ID for edited images
	 * @returns {string}
	 */
		...
		if (typeof window !== 'undefined') {
			try {
				...
				localStorage.setItem(
					LOCAL_STORAGE_GLOBAL_SELECTION_KEY,
					JSON.stringify(globalSelectionToSave)
				);
			} catch (error) {
	...
	return {
		...
		get selectedEditedImage() {
			if (
				editedImages.selectedIndex >= 0 &&
				editedImages.selectedIndex < editedImages.images.length
			) {
		...
		get selectedGeneratedGif() {
			if (
				generatedGifs.selectedIndex >= 0 &&
				generatedGifs.selectedIndex < generatedGifs.gifs.length
			) {
		...
		async getGloballySelectedImageBlob() {
			...
			switch (globalSelection.selectedImageType) {
				...
				case 'edited':
					const editedImage = editedImages.images.find(
						(img) => img.id === globalSelection.selectedImageId
					);
		 ...
		 * @returns {Promise<{editedImagesCount: number, maxEditedImages: number, cacheEntriesCount: number, hasFace: boolean, hasDancerFrame: boolean, storageType: string}>} Storage usage details
		 */
		 ...
		 * @returns {{name: string, hasFace: boolean, faceTimestamp: string, faceCoordinates: FaceCoordinates | null, hasDancerFrame: boolean, dancerFrameTimestamp: string, dancerFrameGifUrl: string, hasEditedImages: boolean, editedImagesCount: number, selectedEditedImageIndex: number}} Summary of all user data and state
		 */
		getUserData() {
			return {
				name: user.name,
